# SECJ1013 - Programming Technique I
👩‍🏫**Lecturer:** Dr. Goh Eg Su  
🏫**Section:** 03

This course introduces the programming basics using the C++ language. I learned how to write, debug and run small programs. My problem-solving abilities also improved as I was given assignments that tested my abilities. The skills that I gained from this course are beneficial to me as I continue with my programming tasks.
<br>

![TYPING](https://github.com/user-attachments/assets/624cc623-5333-482b-9be7-bad5a738800c)

## 📘 [Topics Covered](https://drive.google.com/drive/folders/1Bnq4141G0cmEZoZbuYw4cQD1A8vwmCtv?usp=drive_link)
*Topic 01:* Programming Problem - Solving  
*Topic 02:* Elementary Programming  
*Topic 03:* Control Structures  
*Topic 04:* Function  
*Topic 05:* Array  
*Topic 06:* Input and Output  
*Topic 07:* Pointers  
*Topic 08:* Structured Data  
<br>

## 📖 Coursework
|Title|Link|
|---|---|
|Assignment 1|[View](https://github.com/XuanYee06/MyUniversityJourney/blob/df31159798009aacaed8d87a6f9bf6854582fad9/SECJ1013%20-%20Programming%20Technique%20I/Assignment%201.pdf)|
|Assignment 2|[View](https://github.com/XuanYee06/MyUniversityJourney/blob/df31159798009aacaed8d87a6f9bf6854582fad9/SECJ1013%20-%20Programming%20Technique%20I/Assignment%202.cpp)|
|Assignment 3|[View](https://github.com/XuanYee06/MyUniversityJourney/blob/main/SECJ1013%20-%20Programming%20Technique%20I/Assignment%203.cpp)|
|Lab 2|[View](https://github.com/XuanYee06/MyUniversityJourney/tree/df31159798009aacaed8d87a6f9bf6854582fad9/SECJ1013%20-%20Programming%20Technique%20I/Lab%202)|
|Lab 3|[View](https://github.com/XuanYee06/MyUniversityJourney/tree/c985fd961263c16e7432ccb90136eeeeaf9c30ff/SECJ1013%20-%20Programming%20Technique%20I/Lab%203)|
|Lab 4|[View](https://github.com/XuanYee06/MyUniversityJourney/tree/main/SECJ1013%20-%20Programming%20Technique%20I/Lab%204)|
<br>

## 🧠 Reflection
During this course, I gained knowledges about algorithms, flow of control (loops and conditions) and modular programming through functions. I learned how to use programming languages like C++ to solve problems patiently. In my opinion, I believe that mini projects like a simple calculator and a simple game like Tic-Tac-Toe would improve my programming skills and logical thinking.  It always feels great and satisfying that my program was able to run without any errors. At the end of the course, I was able to write programs for data processing, calculations and solving problems. I hope I can improve my logical thinking and problem solving skills throughout this course so I am able to solve more complicated coding problems and challenges.
